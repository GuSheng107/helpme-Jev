...
│   │   ├── translation.py          # 【新】注释翻译：中文 → 英文 + 方括号语用注释；枚举走本地映射
...
① 输入         用户粘贴对方的话（可附图片 → 原样交多模态 LLM，系统不做 OCR）
      ↓
② 上下文装配    【P2 最小版】只取最近 N 条消息
                【P3 起】再加记忆 + 人设档案 + 滚动摘要（双预算裁剪，见 §8.5）
      ↓
③ 翻译桥（入）  非英文才走：LLM 译成英文，**并附加方括号语用注释**
                （「注释翻译」，见 §9.6）—— JEV 吃注释版，用户看中文原文
      ↓
④ JEV 判断      场景题目集一次调用 → 输出对「对话者」的各种决策
                （情绪 / 意图 / 危险度 / 该不该实质回复 / 最佳动作 / 她需要什么 / …）
      ├─ 信息不足 → ⑤ LLM 生成追问 → 用户补充 → 回到 ②   【P4；可选，轮次不设上限】
      ↓
⑥ 结果展示      枚举走**本地映射表**转中文（不回译）；自由文本才由 LLM 译回
      ↓
⑦ 人设更新      JEV 建模题（信息足够时自动更新）→ personas（见 §8.6，**P5**）
      ↓
⑧ 用户查看决策 ──【用户手动触发，P4】──→ ⑨ LLM 只起草**用户母语**的 3 条候选（严格依据 JEV 的决策）
      ↓
⑩ JEV 排序      候选经注释翻译后排序；**分数与名次在本地映射成中文**，英文不进界面
      ↓
...
| **JEV** | **装配预算：默认 5000 字符，可配** | 装配器裁剪：从优先级最低者起整块删除，绝不截半句 |

> **官方规格（TypeSafe System One / `jev-1.13.0`，2026-09-20 核对）**：
> - 单请求 **64k token**（state + 全部 questions **合计**）
> - 其中 **state + 单个最长 question ≤ 32k token**（英文约 15 万字符）
> - **仅文本**输入（图片 / 音视频须先转文本）
> - 英文为主，**CJK 明确写着「handled but not equally well」** ← 翻译桥正是补这个短板
> - 已知失败模式：**大而嘈杂的 state 会让无关细节成为干扰项**，
>   官方建议 *"filter first; send only what the question needs"*
>
> **为什么默认只给 5000 字符（远低于上限）**：
> 1. **官方自己把"大 state"列为失败模式** —— 上限高 ≠ 该顶格用；
> 2. 设预算的真正理由是「**JEV 是决策模型，输入越精简判断越准**」；
> 3. 5000 字符已足够装下「记忆 + 人设两份 + 情绪轨迹 + 摘要」。
>
> **历史备注**：此前默认值 1500 抄自 Jarvis v1.3 的 `ContextBuilder.BUDGET_CHARS`，
> 其注释为 *"Deliberately dumb and cheap (v1.3 revision)"* —— 是**省事省算力的工程约定**，
> 与模型能力无关；且 Jarvis 的口径是 `background` + `history` 合计，**不含最近 10 条消息**。
> 皇上指出该概念被写混，已改正。
...
**官方规格（`jev-1.13.0`，2026-09-20 核对，仅供参考，以官网为准）**：

| 项 | 值 |
|---|---|
| 端点 | `POST https://api.typesafe.ai/v1/systemone`（用户可填自建/代理地址） |
| 模型 ID | `jev-1.13.0`；别名 `jev-latest`（SDK 默认）/ `jev-preview` |
| 上下文 | **64k token / 请求**（state + 全部 questions 合计）；**state + 单个最长 question ≤ 32k token** |
| 选项上限 | `choice` ≤ **255** 个选项；`score` 为 **2–10** 档 |
| 语言 | 英文为主，**CJK「handled but not equally well」** → 这就是翻译桥的存在理由 |
| 输入类型 | **仅文本**（图片 / 音视频须先转文本） |
| 延迟 | 官方标称 70–500ms（一次并行 pass） |
| 失败模式 | 字面理解 / 算术计数 / 日期比较 / 多跳推理 / **大而嘈杂的 state** / 对抗性内容 |

> **验收提示**：响应里会带实际作答的版本化 ID，**日志要记下来**；
> 若要按置信度调阈值，应**钉住版本 ID**而不是用 `jev-latest` 别名（别名会漂）。
...
**核心问题**（皇上 2026-09-23 指出）：JEV 要判的是**潜台词**，而**直译会抹掉语用**。

| 中文原文 | 直译 | 丢失了什么 |
|---|---|---|
| 「没怎么。」 | "Nothing much." | 句号的冷淡感、**试探你care不care**这层意图 |
| 「呵呵」 | "Hehe" | 敷衍 / 讽刺语气 |
| 「好好好」 | "Okay okay okay" | 无奈或不满的重复强调 |
| 「没事，你忙吧」 | "It's fine, go ahead." | 「我已经不高兴了」的潜台词 |

**所以采用「注释翻译」**：LLM 译成英文的同时，**用方括号写明语用** ——

```text
Nothing much. [short sentence + trailing period; dismissive; likely testing whether you care]
```

- **送进 JEV 的是「英文译文 + 语用注释」**
- **用户看的仍是中文原文** —— 注释是给模型吃的，不进用户视野（需要时可在日志里折叠查看）

这样 JEV 拿到的是**语义 + 语用**双通道输入，而不是被削平后的字面。

#### 翻译规则（写进 prompt）

1. **英文译文照实翻译**，不做文学加工
2. **紧跟方括号追加语用注释**，包含三类信号：
   - 语气 / 情绪：`dismissive` / `teasing` / `resigned` / `angry` / `cold`
   - 句式特征：`short sentence` / `trailing period` / `repeated chars` / `ellipsis`
   - 交际意图：`likely testing whether you care` / `fishing for comfort` / `changing subject`
3. **标点、emoji、语气词原样保留**在译文里
4. 注释**一行以内**，不写小作文
5. **无特殊语用时不加注释** —— 官方明确"大而嘈杂的 state"是失败模式（无关细节会成干扰项），**空白注释纯属噪声**

#### 三个成本优化（皇上指出：都比换模型有效）

| # | 做法 | 收益 |
|---|---|---|
| 1 | **题目与枚举本就是英文** → 结果层只走**本地映射表** | 省掉结果回译调用；且映射是确定性的，**不会翻错**（尤其枚举值） |
| 2 | **候选只用用户母语**；送去排序的英文由注释翻译单独产生，**不回给界面** | 用户看不到英文；排序分数本地映射成中文 |
| 3 | **日志里中英并排** | 不是省成本，而是**能分清"是翻译错还是判断错"** —— 排障的命门 |

> 其中第 2 条是"起草时双版输出"；第 3 条要求日志页提供**并排对照视图**
> （送 JEV 的英文+注释 vs 用户看到的中文原文）。
...
- **翻译桥职责**：**仅当原文非英文时才触发**（入向：注释翻译）；枚举结果走**本地映射表**，自由文本才回译（见 §9.6）
...
| **P1 配置与连通** ✅ | provider_configs（Key 加密）+ CRUD + **连通性/冒烟测试** + 设置页（含上下文窗口配置） | 填错 URL/Key 有明显报错；填对可通过；JEV 出**健康度** |
| **P2 恋爱判断闭环** ⭐ | builders + **恋爱场景 10 题** + **注释翻译**（中→英 + 方括号语用注释）+ jev_client（协议校验 + 防御性重试 + 降级）+ `/chat/analyze` + **装配器最小版**（只取最近 N 条消息）+ **决策面板** | **粘贴一句话 → 看到危险度 / 真实意图 / 最佳动作 / 她需要什么 / 情绪** —— 产品第一次可用的时刻 |
| **P3 记忆与上下文** | 装配器**扩展**（记忆 + 人设 + 滚动摘要 + 双预算裁剪）+ **记忆治理：复盘管线 / 条目化时序 / 写入决策 / 冲突仲裁** + 记忆管理页 | 复盘变更集可回溯、可撤销；超限裁剪正确 |
| **P4 方案链路 + 追问 + 润色** | LLM 起草 3 条（**仅用户母语**）+ JEV 排序（分数**本地映射成中文**）+ 用户定夺 + `/chat/evaluate` + `/chat/clarify` 追问闭环（含"信息充足度 %"与"跳过追问"）+ `/api/polish` 输入润色 + **全程进度指示** | 候选按占比排序；界面无英文；润色直接替换并可撤回 |
| **P5 人设建模 + 素材导入** | personas + **场景级人设题集** + 自评问卷 + `/personas/build` + 人设页；聊天记录导入（预览确认）+ QA 对导入 + 多模态截图 | 导入前预览确认；低置信度**不覆盖**旧档案；科学性弱的有明示 |
| **P6 通用决策 + 场景化** | `/decide` + 决策工作台页 + 职场场景题目集（**agent-first 生成**）+ 自定义场景 | 三种题型可视化正确；可复制预设并改题 |
| **P7 日志与数据权利** | call_logs(trace_id) + 脱敏 + 日志页（**中英并排对照**）+ 数据导出 / 账号注销 | 日志无 Key 泄漏；traceId 可拉全链路；**能分清"翻译错"还是"判断错"** |

> **⚠️ 阶段顺序已于 2026-09-23 重排（皇上指出）**：
> 原顺序是"**按层做**"（P2 先做装配与记忆，P3 才做 JEV）——
> 但**记忆复盘的原料是判断结果**，JEV 一次没跑过时，复盘无料可复盘；
> **装配器也没东西可装**（只有消息）。按原顺序会**先造一套空转的记忆系统**。
>
> 改为"**按闭环做**"：先把「粘贴一句话 → 看到判断」这条链路打通（P2），
> **让产品第一次能用**；记忆与人设等**有真实判断可沉淀之后**再做。
> —— 这也是"记忆的功能是沉淀判断"这一本质决定的。
...
| 8 | **翻译桥** | 因 JEV 对中文不友好，入参走**注释翻译**（英文译文 + 方括号语用注释）；枚举结果走**本地映射表**，不回译（见 §9.6 / 第 32 条） |
...
| 20 | **数据权利** | 提供**数据导出**与**账号注销**（见 §11 / §13 P7 日志与数据权利） |
...
| 28 | **生成回复的触发方式** | **纯粹手动触发**：分析完先给决策，用户点了才生成候选 —— **不做预热**。候选**只用用户母语**，不附英文版；Jev 的排序分数在本地映射成中文展示（见 §17） |
...
| 31 | **JEV 装配预算** | **默认 5000 字符、可配**。官方上限 32k token（≈15 万英文字符），默认**刻意保守**——官方把"大 state"列为失败模式，且"输入越精简判断越准"（见 §8.5） |
| 32 | **翻译桥用「注释翻译」** | 译成英文的同时**追加方括号语用注释**（语气/句式/交际意图），**JEV 吃注释版、用户看中文原文**；直译会抹掉潜台词（见 §9.6） |
...
| **P1** | `clients/jev_client.py` | 协议校验、4xx 去 background/history 重试、429/5xx 退避、错误码映射、冒烟健康度 |
| **P2** | `clients/translation.py` | **注释翻译**：译文 + 方括号语用注释；无特殊语用时**不加注释**；枚举走本地映射表、不回译 |
| **P2** | `scenarios/questions_romance.py` + builders | 恋爱 10 题结构合法（noul / choice / score）；一次请求能装下 |
| **P2** | `services/chat_service.py`（analyze） | 装配只取最近 N 条；上游失败有明确错误且不崩；跨用户 404 |
| **P3** | `services/context_service.py` | 双预算裁剪顺序（先删最旧、**绝不截半句**）、超限行为 |
| **P3** | `services/memory_service.py` | 四种写入决策、冲突仲裁优先级、复盘变更集**可撤销** |
| **P4** | `/chat/reply` `/chat/evaluate` `/api/polish` | 候选只有母语；排序分数是中文；润色可撤回；追问可跳过 |
| **P5** | `services/import_service.py` | 聊天记录解析（多种格式）、QA 对校验（非法 JSON 必须拒绝） |
| **各阶段** | `api/*` | 端点契约测试（含 `409` / `422` / `PROTOCOL_MISMATCH` 等错误码） |
...
| 7 | 翻译桥语气损耗 | 初版只要求保留标点 / emoji / 语气词；**后被皇上否掉**，改为 §9.6 **注释翻译**（方括号语用注释）+ 日志页**中英并排对照**（排在 P7） |

### 采纳并排入阶段（阶段号已按 2026-09-23 重排，见 §13）

| # | 意见 | 排期与做法 |
|---|---|---|
| 8 | **决策可解释层** | **P4**：LLM 基于「JEV 决策 + 消息」生成"为什么这么判"，UI 标注**由 LLM 解读、仅供参考**。属"表达"而非"决策"，**不破铁律** |
| 9 | **连通性测试 → 冒烟测试 + JEV 健康度** | ✅ **P1 已完成**：内置 5 组标准 case，连通测试时顺带跑出健康度 |
| 10 | **高危险度人文设计** | **P2**（已提前 —— 它本就是决策面板的一部分）：`danger_level ≥ 8` 时面板首屏不再是"3 条候选"，<br>而是「**这已超出文字回复能解决的范围，建议当面或电话沟通**」 |
| 11 | **决策效果闭环** | **P3 之后**：复盘管线捕捉「上次定夺 → 本次情绪变化」的关联，让人设/记忆更新带上因果 |

### 小项

| # | 意见 | 落实 |
|---|---|---|
| 12 | 串行调用延迟（实为 **7 次**） | **P4**：候选不再附英文版（2026-09-23 改）；送排序前单独做注释翻译；<br>前端**全程逐步进度指示**（翻译 / 判断 / 起草 / 排序 走到哪一步） |
| 13 | 追问地狱 | **P4**：面板常显「**信息充足度 xx%**」，并**永远保留「跳过追问，直接出结果」** —— 拍板在用户，不在 JEV |
| 14 | 依赖度提醒 | **P5**：统计「候选直接采用率 vs 手动改写率」，**只展示不说教** |
| 15 | token 存 localStorage 的 XSS 面 | 已在 §5 标注该取舍（自部署小圈子可接受；如需加固改 httpOnly cookie + CSRF） |

> **顺带记录**：第 9 条的"**健康度未达标前，排序条数 UI 锁死 3 条**"随 P1 完成已具备数据基础，
> 具体锁定逻辑排在 **P4**（排序功能上线时一并生效）。
...
### 追加：翻译桥改为「注释翻译」（皇上 2026-09-23 指出）

皇上否定了"只保留标点/emoji/语气词"这一版 —— **那只解决形态，不解决潜台词丢失**：

> 「JEV 要判的是潜台词，直译会把『没怎么。』翻成 Nothing much.，试探感就没了。
> 更好的做法是**注释翻译，不是文学翻译**：LLM 译成英文的同时，在括号里写明语用。」
...
- **JEV 吃「英文 + 方括号语用注释」**；**用户仍看中文原文**
- 注释含三类信号：语气情绪 / 句式特征 / 交际意图
- **无特殊语用时不加注释** —— 官方明确"大而嘈杂的 state"是失败模式，空注释纯属噪声

**皇上同时点出三件"比换模型有用"的事**，均已写入 §9.6：

| # | 做法 | 收益 |
|---|---|---|
| 1 | 题目与枚举本就是英文 → 结果只走**本地映射表** | 省调用，且映射确定、**不会翻错** |
| 2 | **起草候选时一次给中英两版** | 省掉"候选再译英送去排序"这一次往返 |
| 3 | **日志里中英并排** | 分清"翻译错"还是"判断错" —— 排障命门 |

### 已定（1 条）

| # | 问题 | 结论 |
|---|---|---|
| 9 | `default_admin_password` 有源码级默认值 | **皇上 2026-09-23 决定：保持现状，不改**。<br>即继续开箱即用（`admin` / `helpme-jev-admin-2026!`），由**首次登录强制改密**兜底，不引入"未显式配置则拒绝启动"。 |
...
- 因此第 12 条的优化只取**两点**：
   1. 候选**只用用户母语**。送去 Jev 排序的英文由注释翻译产生，分数在本地映射成中文，**英文不进界面**（2026-09-23 改，不再一次出中英两版）
