...
⑦ 人设更新      JEV 建模题（信息足够时自动更新）→ personas（见 §8.6，**P5**）
      ↓
⑧ 用户查看决策 ──【用户手动触发，P4】──→ ⑨ LLM 一次起草中英双版候选（严格依据 JEV 的决策）
      ↓
⑩ JEV 排序      直接用英文版候选做优劣决策（不再二次翻译）
      ↓
...
| 2 | **起草候选时一次输出中文 + 英文两版** | 省掉"候选再译英送去排序"这一次往返 |
...
| **P4 方案链路 + 追问 + 润色** | LLM 起草 3 条（**一次输出中英双版**）+ JEV 排序 + 用户定夺 + `/chat/evaluate` + `/chat/clarify` 追问闭环（含"信息充足度 %"与"跳过追问"）+ `/api/polish` 输入润色 + **全程进度指示** | 候选按占比排序；润色直接替换并可撤回 |
...
| **P4** | `/chat/reply` `/chat/evaluate` `/api/polish` | 候选一次出中英双版；排序占比；润色可撤回；追问可跳过 |
...
| # | 意见 | 落实 |
|---|---|---|
| 12 | 串行调用延迟（实为 **7 次**） | **P4**：LLM 起草时**一次输出中英双版**（省一次翻译往返）；<br>前端**全程逐步进度指示**（翻译 / 判断 / 起草 / 排序 走到哪一步） |
